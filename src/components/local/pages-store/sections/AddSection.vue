...
const validationRules = ref({
  name: {
    en: { minLength: minLength(1) },
    ar: { minLength: minLength(1) },
  },
  title: {
    tar: { minLength: minLength(1) },
    ten: { minLength: minLength(1) },
  },
  desc: {
    dar: { minLength: minLength(1) },
    den: { minLength: minLength(1) },
  },
  img: {},
  alt: {
    aar: { minLength: minLength(1) },
    aen: { minLength: minLength(1) },
  },
});
  ...
  if (result) {
    if (typeof formData.value.img == "object") {
        ...
        .then(async (res) => {
          await usePageStore()
            .editSection(props.itemData.id, {
              ...
              "pages[]": props.itemData.pages.map((item) => item.id),
            })
            .then(async () => {
              await useItemsStore().getItems("slider", "home");
              closeModal();
      ...
      const res = await usePageStore()
        ...
        .then(async () => {
          await usePageStore().getSections();
          closeModal();
        })
        .finally(() => {
          isLoading.value = false;
