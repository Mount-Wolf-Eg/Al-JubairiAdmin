<template>
  <div>
    <ReusTable
      :header="['', 'Slide title', 'image', 'Created', 'Status', 'Action']"
    >
      <template #table>
        <tr v-for="partner in aboutSlider" :key="partner.id">
          <td>
          ...
          <td
            @click="
              router.push({ name: 'aboutInfo', params: { id: partner.id } })
            "
          >
            <img
              class="mx-2"
...
const { aboutSlider, slider, singleSlide } = storeToRefs(sliderStore());
...
watch(
  () => filteredData.value,
  async (newVal) => {
    if (newVal.length > 0 && newVal[0].slider_type == "about_slider") {
      aboutSlider.value = newVal;
    } else {
...
onMounted(async () => {
  ...
  // await sliderStore().getSingleSliderType("about_slider");
});

const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "about_slider");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "about_slider");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const editPartner = async (partnerId) => {
  ...
  if (res) {
    emit("editSlider", singleSlide.value);
  }
};
</script>
