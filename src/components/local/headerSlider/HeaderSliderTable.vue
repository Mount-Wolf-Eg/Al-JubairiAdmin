<template>
  <div>
    ...
    >
      <template #table>
        <tr v-for="partner in headerSlider" :key="partner.id">
          <td>
          ...
          <td
            @click="
              router.push({ name: 'SliderInfo', params: { id: partner.id } })
            "
          >
            <img
              class="mx-2"
...
const { headerSlider, slider, singleSlide } = storeToRefs(sliderStore());
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "header_slider");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "header_slider");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
