<template>
  <div>
    ...
    >
      <template #table>
        <tr v-for="pack in allPackages" :key="pack.id">
          <td>
          ...
          >
            {{ pack.deleted_at == null ? "Active" : "Suspended" }}
          </td>
...
watch(
  ...
  async (newVal) => {
    if (newVal.length) {
      ...
      await useProductStore().getAllPackages();
    }
  }
);
onMounted(async () => {
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await useProductStore().restorePackage({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await useProductStore().destroyPackage({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const removePackage = async (id) => {
  const res = await useProductStore().deletePackage({ id: id });
  await useProductStore().getAllPackages();
};

const editPackage = async (pack) => {
  let res = await useProductStore().getPackage({ id: pack });
  if (res) {
