...
const { allCategories } = storeToRefs(useProductStore());
// validation
...
watch(
  ...
  () => {
    ...
    formData.value.imgDescription.arrr = props.package.image_alt.ar;
    formData.value.imgDescription.ennn = props.package.image_alt.en;

    // formData.value.img2 = props.package.second_image;
    // formData.value.imgDescription2.amr = props.package.second_image_alt.ar;
    // formData.value.imgDescription2.emn = props.package.second_image_alt.en;

    // formData.value.features = props.package.features;

    formData.value.content.car = props.package.content.ar;
    formData.value.content.cen = props.package.content.en;

    formData.value.attachments = props.package.attachments;

    formData.value.appStore = props.package.settings?.app_store_url;
    formData.value.playStore = props.package.settings?.play_store_url;
    formData.value.webUrl = props.package.settings?.site_url;

    formData.value.category = props.package.category_id;

    categoryData.value.value = props.package.category_id;
    formData.value.imgs = props.package.attachments;
    categoryData.value.placeholder = allCategories.value.find(
      (c) => c.id == props.package.category_id
    ).name;
  }
);
...
const resetFormData = () => {
  ...
  document.getElementById("addProduc").reset();
};
...
onMounted(async () => {
  await useProductStore().getAllCategories();
...
const addPack = async () => {
  ...
  if (result) {
    const res = await useProductStore().addPackage({
...
const updatePack = async () => {
  ...
  const result = await validationObj.value.$validate();
  if (result) {
    const res = await useProductStore().updatePackage({
      "name[ar]": formData.value.name.ar,
      "name[en]": formData.value.name.en,
      "description[ar]": formData.value.packContent.arr,
      "description[en]": formData.value.packContent.enn,
