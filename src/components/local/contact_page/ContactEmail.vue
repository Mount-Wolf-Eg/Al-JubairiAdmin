        ...
        <span class="col">
          <span class="d-flex gap-3">
            <InptField
              ...
              :holder="'email'"
              :label="'Contact Email'"
              :appear="checkErrName(['ContactEmail']) ? 'err-border' : ''"
            ></InptField>
            <button
              v-if="!isLoading"
          ...
          <span
            ...
            :key="i"
            ><span v-if="err.$property == 'ContactEmail'" class="err-msg">
              {{ err.$message }}
            </span></span
          >
        </span>
...
onMounted(async () => {
  ...
  formData.value.ContactEmail = allContacts.value.contact_mail;
});
...
const handleSetting = async () => {
  ...
  if (result) {
    ...
    let res = await contactSettings().updateContacts({
      "settings[contact_mail]": formData.value.ContactEmail,
    });
    if (res) {
      await contactSettings().getAllContacts();
    }
