      ...
      <div class="modal-content add-admin-modal">
        ...
        <div class="modal-body d-flex flex-wrap justify-content-between">
          ...
          <span class="row w-100">
            <!-- Name -->
            <span class="col center-row gap-5 mt-3 mb-4">
              <span class="center-row gap-1">
                ...
                />
                <label class="rad-title" for="left">Services Left</label>
              </span>
              ...
              <span class="center-row gap-1">
                <input
                  ...
                  id="right"
                  name="services_right"
                  value="services_right"
                  v-model="formData.typ"
                />
                <label class="rad-title" for="right">Services Right</label>
              </span>
              <span class="center-row gap-1">
                <input
                  type="radio"
                  id="bottom"
                  name="services_bottom"
                  value="services_bottom"
                  v-model="formData.typ"
                />
                <label class="rad-title" for="bottom">Services Bottom</label>
              </span>
            </span>
            <span
              class="center-row justify-content-start"
              ...
              v-for="(err, i) in validationObj.$errors"
              :key="i"
              ><span v-if="err.$property == 'typ'" class="err-msg">
                {{ err.$message }}
              </span></span
            >
          </span>
...
watch(
  ...
  () => {
    ...
    formData.value.typ = props.partner.slider_type;
  }
);
...
const resetFormData = () => {
  ...
  document.getElementById("serSlider").reset();
};
...
const addPartner = async () => {
  ...
  if (result) {
    const res = await sliderStore().addslider(
      {
        slider_type: formData.value.typ,
