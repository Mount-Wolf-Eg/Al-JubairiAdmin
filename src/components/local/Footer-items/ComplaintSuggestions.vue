...
onMounted(async () => {
  await useItemsStore().getItems("", "suggestions", "suggestions");
  formData.value.desc.ar = allItems.value.find((el) => el.id == 291)?.ar?.desc;
  formData.value.desc.en = allItems.value.find((el) => el.id == 291)?.en?.desc;
...
const handleSetting = async () => {
  ...
  if (result) {
    ...
    await useItemsStore().editItem("291", {
      _method: "PUT",
