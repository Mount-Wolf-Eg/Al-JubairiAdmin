<template>
  <main v-if="!pageLoading">
    <ReusTable
      :header="['', 'Image', 'Name', 'email', 'Phone', 'Created', 'Actions']"
    >
      <template #table>
        <tr v-for="item in allAdmins" :key="item.id">
          <td>
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await useAdminStore().toggle(id);
    if (!res) {
...
const remove = async (id) => {
  await useAdminStore().deleteItem(id);
  await useAdminStore().getAllAdmins();
};

const edit = async (id) => {
  let res = await useAdminStore().getSingleItem(id);

  if (res) {
    emit("editItem", admin.value);
  }
};
</script>
